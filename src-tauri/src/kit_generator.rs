...
mod tests {
    ...
    fn test_generate_kit_with_memory_enabled() {
        ...
        let mcp_config = McpConfig {
            url: "http://host.docker.internal:9100/mcp?token=secret-token-123".to_string(),
    ...
    fn test_generate_kit_with_memory_and_additional_mcp() {
        ...
        let mcp_config = McpConfig {
            url: "http://host.docker.internal:9200/mcp?token=mem-token".to_string(),
        ...
        assert!(content.contains("host.docker.internal:9200/mcp?token=mem-token"));
        ...
        fn arb_mcp_config() -> impl Strategy<Value = Option<McpConfig>> {
            prop_oneof![
                ...
                (arb_port(), "[a-zA-Z0-9]{10,30}").prop_map(|(port, token)| {
                    Some(McpConfig {
                        url: format!("http://host.docker.internal:{}/mcp?token={}", port, token),
                        bearer_token: token,
                        port,
                    })
                }),
