...
**Description:** A dedicated "Repo Sync" section that manages git remote synchronization for workspace repositories using an isolated two-directory architecture. The AI agent works in a remote-free workspace (no credentials, no remote URLs visible). A separate host-side mirror directory holds the real remote configuration and credentials. All sync operations are user-initiated and run host-side.

**Core security principle:** The sandbox workspace has NO git remotes. The agent can only commit locally. Remote URLs, credentials, and sync state live exclusively in a host-side mirror directory that is never mounted into any sandbox. The user explicitly decides when to sync in either direction.

**Architecture — Two directories per managed repo:**

| Location | Visible to sandbox? | Has remotes? | Purpose |
|----------|---------------------|--------------|---------|
| Sandbox workspace (mounted) | Yes | No — all remotes stripped | Agent commits here |
| Host-side mirror (`<mirrors-dir>/<persona>/<project>/`) | No | Yes — real remote configured | Repo Sync operates here |

```
GitHub/GitLab ←→ [Host-side mirror] ←→ [Sandbox workspace]
                       ↑                        ↑
                  Repo Sync manages        Agent works here
                  real remote + creds      No remotes, local commits only
                  User-initiated sync      User-initiated sync
```

**Mirrors directory:**
- User-configurable location (default: `~/.local/share/beachead/mirrors/`)
- Structure: `<mirrors-dir>/<persona-name>/<project-folder-name>/`
- Project folder name matches the top-level directory containing `.git` (e.g., if workspace is `/home/user/projects/my-app`, mirror is `<mirrors-dir>/<persona>/my-app/`)
- Collision handling: if two personas have projects with the same folder name, persona namespacing prevents conflicts

**Workspace modes (persona configuration):**

| Mode | Description | Use case |
|------|-------------|----------|
| **Direct mount** (current default) | Host workspace mounted directly into sandbox. Agent sees `.git/config`, remotes, `.env`, etc. | Trusted agents, simple setups, no isolation needed |
| **Isolated clone** | Beachead creates a local clone, strips all remotes, mounts the clone. Agent commits locally only. Repo Sync manages upstream via host-side mirror. | Untrusted agents, credential isolation, controlled sync |

User selects mode per-workspace in persona configuration. Isolated clone is recommended when Repo Sync is enabled.

**Setup flow for existing repos (isolated clone mode):**

1. User configures persona workspace with "Isolated clone" mode
2. At first session start, Beachead clones the workspace to the sandbox workspace directory, stripping all remotes
3. Beachead creates the host-side mirror (clone from original workspace, preserving remotes)
4. The original workspace remote URL is stored in the DB (auto-detected from `.git/config`)
5. User can edit remote URL, provider, credentials, and branch strategy in Repo Sync UI
6. Sandbox mounts the remote-free clone; mirror stays on host only

**Setup flow for sandbox-originated projects:**

1. Agent runs `git init` and commits code inside the sandbox workspace
2. Beachead detects new repo (scan on session stop or user-triggered scan)
3. User chooses via Repo Sync UI:
   - **"Link to remote"**: User provides remote URL → Beachead creates host-side mirror (cloned from sandbox workspace), adds user-provided remote to mirror only, strips any remote the agent may have added from the sandbox copy
   - **"Keep local only"**: Beachead creates host-side mirror (cloned from sandbox workspace, no remote added). Just a backup/sync point. User can add a remote later.
4. Sandbox workspace remains remote-free in both cases

**Sync operations (all user-initiated, all host-side):**

| Operation | Direction | Trigger | Mechanism |
|-----------|-----------|---------|-----------|
| **Pull from agent** | Sandbox → Mirror | User clicks "Pull from agent" | `git -C <mirror> fetch <sandbox-workspace-path>` then merge/rebase |
| **Push to remote** | Mirror → GitHub | User clicks "Push" | `git -C <mirror> push origin <branch>` with host credentials |
| **Fetch from remote** | GitHub → Mirror | User clicks "Fetch" | `git -C <mirror> fetch origin` with host credentials |
| **Push to agent** | Mirror → Sandbox | User clicks "Push to agent" | `git -C <sandbox-workspace> pull <mirror-path>` (local paths, no network) |

All four operations are on-demand only. The user decides when to sync in each direction.

**Notifications (informational only, no auto-sync):**
- Badge on Repo Sync sidebar item when sandbox workspace has commits not yet pulled to mirror
- Badge/indicator when remote has new commits not yet fetched to mirror
- Periodic check (configurable interval) for new commits in both directions
- Optional desktop notification when changes detected
- Notifications inform — they never trigger automatic sync

**Per-managed-repo configuration:**
- Remote URL (auto-detected from original `.git/config` on setup, editable)
- Remote provider (GitHub, GitLab, Bitbucket, custom)
- Credentials (per-repo — different repos may use different accounts/tokens)
- Branch strategy (user's choice):
...
- Sync mode: "Local only" (no remote) or "Remote" (with configured URL)
...
**Commit review before push to remote:**
- Show commit list with diffs for unpushed changes in the mirror
...
- Before allowing push to remote, scan staged files against common secret/artifact patterns
- Warn if potential secrets detected (`.env`, private keys, tokens in code)
- Configurable: block push or warn-only

**Pull request creation:**
- Out of scope for initial implementation — user creates PRs directly on GitHub/GitLab after pushing to a feature branch
- Future enhancement: optional "Create PR" button using `gh pr create` / GitLab API

**UI structure:**
- Sidebar: "Repo Sync" menu item (below Sessions, above Docker)
- Main view: list of managed repos grouped by persona
- Per-repo view: sync status (commits ahead/behind in each direction), commit list, sync buttons (Pull from agent, Push to remote, Fetch from remote, Push to agent), settings gear
- Settings panel per repo: remote URL, credentials, branch strategy, attribution, sync mode

**Implementation notes:**
- All git operations run on the host side (not inside sandbox) — use `std::process::Command` with `git` CLI or `git2` crate
- Credentials passed via `GIT_ASKPASS` helper or credential.helper config (never written to any workspace `.git/config`)
- Mirror directory path stored in app settings (user-configurable)
- DB tables: `managed_repos` (id, persona_id, sandbox_workspace_path, mirror_path, remote_url, provider, branch_strategy, attribution_mode, sync_mode, created_at), `repo_credentials` (id, repo_id, credential_ref — pointer to keyring entry or encrypted file entry)
- On persona deletion: prompt user to keep or delete associated mirrors
- On session start (isolated clone mode): verify sandbox workspace is in sync with mirror, warn if diverged
- No commercial equivalent exists that combines sandboxed AI agent isolation with host-side git remote management via isolated mirrors

**Reference implementation:**
- **VS Code git extension** (MIT license): https://github.com/microsoft/vscode/tree/main/extensions/git/src
  - `git.ts` — Battle-tested git CLI wrapper with output parsing, error handling, exit code interpretation. Use as reference for building the Rust git CLI wrapper.
  - `askpass.ts` / `credentials.ts` — Cross-platform `GIT_ASKPASS` helper implementation using IPC. Adapt this pattern for the credential helper that intercepts auth requests without storing credentials in `.git/config`.
  - `repository.ts` — Ahead/behind calculation, working tree change detection, status parsing. Reference for sync state tracking between mirror and sandbox.
  - `merge.ts` — Conflict detection patterns. Reference for detecting diverged histories before sync operations.
  - Note: VS Code's git wrapper is TypeScript; the backend implementation will be Rust, but the logic and edge case handling translates directly. Do not copy UI code (VS Code extension API doesn't apply).
